...
class BasicNavigator(Node):
    ...
    def goThroughPoses(self, poses):
        ...
        self.info('Navigating with ' + str(len(goal_msg.path.poses)) + ' goals.' + '...')
        ...
        if not self.goal_handle.accepted:
            self.error('Goal with ' + str(len(poses.poses)) + ' poses was rejected!')
            return False
    ...
    def isNavComplete(self):
        if not self.result_future:
            ...
            return True
        rclpy.spin_until_future_complete(self, self.result_future, timeout_sec=0.005)
